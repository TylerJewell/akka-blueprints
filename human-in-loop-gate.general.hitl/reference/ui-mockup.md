...
5. **App UI** — submit a request; live list of actions via SSE; per-action Approve/Reject buttons shown only when `status == PROPOSED` and `proposalSummary` is present. Executed actions show the outcome; rejected actions show the reason.
...
- **Request input** — single text field + Submit button → `POST /api/action-request`.
- **Actions list** — one card per action: proposal summary, risk level badge, rationale excerpt, status badge, and (for `PROPOSED` actions) an approver-name field, a comment field, Approve and Reject buttons. Reject reveals a reason field.
- **Status badge** — `PROPOSED` neutral, `APPROVED` yellow, `EXECUTED` green, `REJECTED` muted red.
- **Risk level badge** — `LOW` green, `MEDIUM` yellow, `HIGH` red; shown on all non-initial cards.
