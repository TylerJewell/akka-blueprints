...
Browser title: `<title>Akka Sample: Parallelization Workflow (Sectioning + Voting)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Parallelization Workflow (Sectioning + Voting)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: the supervisor partitions the job (sectioning) or prepares a shared prompt (voting); workers run in parallel; the supervisor aggregates all results with an output guardrail.
...
A form with a `prompt` text input, a `mode` selector (SECTIONING / VOTING), and a Submit button (`POST /api/jobs`). Below it, a live list of jobs streamed from `/api/jobs/sse`. Each row shows the prompt (truncated to 80 chars), the mode, a status pill (PARTITIONING / IN_PROGRESS / AGGREGATED / PARTIAL / BLOCKED), and the eval score when present. Click a row to expand: for SECTIONING mode show the list of section results with their indices; for VOTING mode show each vote result with its workerIndex and confidence score; in both cases show the aggregated answer and the eval rationale.
