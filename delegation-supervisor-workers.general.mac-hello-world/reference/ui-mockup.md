...
Browser title: `<title>Akka Sample: Multi-Agent Hello World</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Multi-Agent Hello World", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose into writing spec + advisory spec, parallel fan-out to GreetingWriter and ActionAdvisor, compose final response.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. Zero controls defined for this baseline; the tab renders an empty-state message: "No governance controls are defined for this baseline. Add controls in `eval-matrix.yaml` when extending to a production use case."

## Tab 5 — App UI

A form with a `name` text input, a `context` text input, and a Submit button (`POST /api/greetings`). Below it, a live list of requests streamed from `/api/greetings/sse`. Each row shows the name, the context snippet, and a status pill (PENDING / IN_PROGRESS / COMPLETED / DEGRADED). Click a row to expand the draft message, the follow-up action, and the composed response.
