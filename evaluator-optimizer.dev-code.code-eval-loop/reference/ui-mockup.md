...
- Eyebrow: `Overview`.
- Headline: `Code Assistant <span class="accent">(AlphaCodium-style)</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a problem statement in the App UI tab.
  2. Watch the solution enter `GENERATING`, then `VERIFYING`, then either `PASSED` or `EXHAUSTED`.
  3. Expand the solution to see every attempt's code, the sandbox verdict, the verifier's verdict, and the verifier's diagnostics.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a generator produces code, a verifier runs checks and tests, the workflow loops until all tests pass or the retry budget runs out.
...
- ID badges coloured per mechanism (guardrail = red, eval-event = blue, ci-gate = orange).
...
- Form card: textarea labelled "Problem statement", select field labelled "Language" (default "java"), `Submit` button (yellow).
- Live list: cards per solution; left border coloured by status:
  - `GENERATING` — muted grey
  - `VERIFYING` — blue
  - `PASSED` — green
  - `EXHAUSTED` — red
- Header row: statement (first 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The code in a syntax-highlighted monospaced block with a language badge.
  - The sandbox verdict pill (`OK` green, `FORBIDDEN_IMPORT` red, `UNSAFE_SYSCALL` red with the detail text).
  - The verifier's verdict pill (`PASS` green, `FAIL` orange) plus the score badge (e.g., `60/100`).
  - The verifier's diagnostics list when present, plus the `overallRationale`.
- Terminal block:
  - On `PASSED`: the passed code in a highlighted box plus a "passed on attempt N" caption.
  - On `EXHAUSTED`: the best-scoring attempt's code plus the structured `exhaustionReason`.
