# UI mockup — sql-gen-validator-loop
...
- Eyebrow: `Overview`.
- Headline: `SQL <span class="accent">Generator</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a natural-language question in the App UI tab.
  2. Watch the request enter `GENERATING`, then `VALIDATING`, then either `ACCEPTED` or `FAILED_FINAL`.
  3. Expand the request to see every attempt's SQL, the guardrail verdict, the validator's verdict, and the validator's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a generator produces SQL, a validator checks it against the schema, the workflow loops until the query passes or the retry ceiling is reached.
...
- Eyebrow: `App UI`. Headline: `Submit a question. <span class="accent">Watch a query land.</span>` Subtitle: `The simulator also drips a question every 60 s so the page is never empty.`
- Form card: text field labelled "Question", schema name field labelled "Schema" (default `default_schema`), `Submit` button (yellow).
- Live list: cards per request; left border coloured by status:
  - `GENERATING` — muted grey
  - `VALIDATING` — blue
  - `ACCEPTED` — green
  - `FAILED_FINAL` — red
- Header row: question (first 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The generated SQL in a monospaced block with syntax highlighting.
  - The guardrail verdict pill (`OK` green, `MUTATION_FORBIDDEN` red with the detail text).
  - The validator's verdict pill (`VALID` green, `INVALID` orange) plus the score badge.
  - The validator's notes (overallRationale + bullets) when present.
- Terminal block:
  - On `ACCEPTED`: the accepted SQL in a highlighted monospaced box plus a "best of N attempts" caption.
  - On `FAILED_FINAL`: the best-scoring attempt's SQL plus the structured `failureReason`.
