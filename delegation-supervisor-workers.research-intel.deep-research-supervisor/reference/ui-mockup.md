...
Browser title: `<title>Akka Sample: Deep Research Supervisor</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 -- Overview

Eyebrow "Overview" + headline "Deep Research Supervisor", **no subtitle**. Four cards:

- **Try it** -- the single command `/akka:build`. No env-var export block.
- **How it works** -- three lines: decompose into subqueries, parallel search/extract/summarise per subquery, synthesise with a citation guardrail.
...
A form with one `question` text input and a Submit button (`POST /api/reports`). Below it, a live list of reports streamed from `/api/reports/sse`. Each row shows the question (truncated to 80 chars), a status pill (PLANNING / IN_PROGRESS / SYNTHESISED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand the per-subquery summaries, the executive summary, the citation list, and the eval rationale.
