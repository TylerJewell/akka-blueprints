...
Browser title: `<title>Akka Sample: Marketing Strategy Team</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Marketing Strategy Team", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose the objective, parallel fan-out to MarketResearcher + AudienceTargeter + MessageStrategist + ChannelPlanner, assemble with a compliance guardrail.
...
A form with one `objective` text input and a Submit button (`POST /api/campaigns`). Below it, a live list of briefs streamed from `/api/campaigns/sse`. Each row shows the objective (truncated to 80 chars), a status pill (PLANNING / IN_PROGRESS / ASSEMBLED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand market insights, audience profile, messaging guide, channel plan, the assembled executive summary, and the eval rationale.
