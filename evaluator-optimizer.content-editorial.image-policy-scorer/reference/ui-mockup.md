...
- Eyebrow: `Overview`.
- Headline: `Image Policy <span class="accent">Scorer</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a prompt in the App UI tab.
  2. Watch the image enter `GENERATING`, then `SCORING`, then either `APPROVED` or `REJECTED_FINAL`.
  3. Expand the image to see every attempt's description, the safety gate verdict, the scorer's verdict, and the policy notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a generator describes, a scorer evaluates, the workflow loops until the policy criteria are met or the retry ceiling is reached.
...
- Each `.qb` block rendered with the question text and the answer chips/textareas/list widgets in their selected state per `risk-survey.yaml`.
...
- Eyebrow: `App UI`. Headline: `Submit a prompt. <span class="accent">Watch an image land.</span>` Subtitle: `The simulator also drips a prompt every 60 s so the page is never empty.`
- Form card: text field labelled "Prompt", select labelled "Audience tier" (options: general / mature / children, default general), `Submit` button (yellow).
- Live list: cards per image; left border coloured by status:
  - `GENERATING` — muted grey
  - `SCORING` — blue
  - `APPROVED` — green
  - `REJECTED_FINAL` — red
- Header row: prompt text (first 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The description text in a monospaced block, plus `contentCategory` and `brandSafetySignal` chips.
  - The safety gate verdict pill (`OK` green, `PROHIBITED_SIGNAL` red with the detail text).
  - The scorer's verdict pill (`PASS` green, `FAIL` orange) plus the score badge.
  - The policy notes (overallRationale + bullets) when present.
- Terminal block:
  - On `APPROVED`: the approved description in a highlighted box plus a "best of N attempts" caption.
  - On `REJECTED_FINAL`: the highest-scoring attempt's description plus the structured `rejectionReason`.
