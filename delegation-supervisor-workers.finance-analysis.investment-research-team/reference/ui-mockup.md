...
Browser title: `<title>Akka Sample: Investment Research Multi-Agent</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Investment Research Multi-Agent", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: plan the research request, parallel fan-out to EquityAnalyst + MarketScout, synthesise with a sector content sanitizer.
...
A form with a `ticker` text input, a `question` text input, and a Submit button (`POST /api/reports`). Below it, a live list of reports streamed from `/api/reports/sse`. Each row shows the ticker, the question (truncated), a status pill (PLANNING / IN_PROGRESS / PUBLISHED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand fundamentals facts, sentiment signals, the synthesised summary, and the eval rationale.
