...
Browser title: `<title>Akka Sample: Financial Assistant (Multi-Agent)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Financial Assistant (Multi-Agent)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: assign work to three specialists in parallel (market researcher, portfolio planner, report drafter); coordinator synthesises all three outputs; a finance-sector sanitizer and output guardrail vet the report before it is published.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. The label column carries the control id and a colored mechanism pill (`sanitizer` orange, `guardrail` red, `eval-event` blue). Click a row to expand name, rationale, and implementation. Three controls: S1, G1, E1.

## Tab 5 — App UI

A form with one `query` text input and a Submit button (`POST /api/financial`). Below it, a live list of reports streamed from `/api/financial/sse`. Each row shows the query, a status pill (DRAFTING / IN_REVIEW / PUBLISHED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand market data points, planning assessment guidance, the report narrative, consolidated executive summary, and the eval rationale.
