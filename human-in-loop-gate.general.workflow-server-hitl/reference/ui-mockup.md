...
5. **App UI** — submit a job request; live list of jobs via SSE; per-job Approve/Reject buttons shown only when `status == ANALYZED` and `findings` is present. Completed jobs show the output; rejected jobs show the reason.
...
- **Job request input** — single textarea + Submit button → `POST /api/jobs`.
- **Jobs list** — one card per job: status badge, risk level badge (color-coded LOW/MEDIUM/HIGH), findings preview, and (for `ANALYZED` jobs) an approver-name field, a note field, Approve and Reject buttons. Reject reveals a reason field.
- **Status badge** — `ANALYZED` neutral, `APPROVED` yellow, `COMPLETED` green, `REJECTED` muted red.
- **Risk badge** — `LOW` green, `MEDIUM` yellow, `HIGH` red. Visible whenever `riskLevel` is present.
