# UI mockup — brand-presentation-builder
...
- Eyebrow: `Overview`.
- Headline: `Brand-Aligned <span class="accent">Presentations</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a brief in the App UI tab.
  2. Watch the presentation enter `BUILDING`, then `REVIEWING`, then either `APPROVED` or `REJECTED_FINAL`.
  3. Expand the presentation to see every attempt's slide set, the guardrail verdict, the reviewer's verdict, and the brand feedback.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a builder agent drafts a slide set, a brand reviewer agent scores it, the workflow loops until approval or the retry ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a brief. <span class="accent">Watch a presentation land.</span>` Subtitle: `The simulator also drips a brief every 60 s so the page is never empty.`
- Form card: text field labelled "Topic", text field labelled "Target audience" (default "general"), numeric field labelled "Slides" (default 5), numeric field labelled "Words per slide" (default 80), `Submit` button (yellow).
- Live list: cards per presentation; left border coloured by status:
  - `BUILDING` — muted grey
  - `REVIEWING` — blue
  - `APPROVED` — green
  - `REJECTED_FINAL` — red
- Header row: topic (first 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The slide set as an expandable accordion — each slide shows its title, body text, and word count in a compact block.
  - The guardrail verdict pill (`OK` green, `OVER_WORD_LIMIT` red with the detail text naming offending slides).
  - The reviewer's verdict pill (`APPROVE` green, `REVISE` orange) plus the score badge.
  - The brand feedback (overallRationale + bullets) when present.
- Terminal block:
  - On `APPROVED`: the approved slide set in a highlighted accordion plus a "best of N attempts" caption.
  - On `REJECTED_FINAL`: the best-scoring attempt's slide set plus the structured `rejectionReason`.
