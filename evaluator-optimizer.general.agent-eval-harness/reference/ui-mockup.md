...
- Eyebrow: `Overview`.
- Headline: `Agent Eval <span class="accent">Harness</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Trigger a run from the App UI tab (select a suite and threshold).
  2. Watch the run enter `PENDING`, then `RUNNING`, then either `PASSED` or `FAILED`.
  3. Expand the run to see each case's response, the judge's verdict, and the judge's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: an evaluator agent drives each test case through the target agent, a judge agent scores each response, the workflow aggregates accuracy and applies a configurable gate.
...
- Eyebrow: `App UI`. Headline: `Trigger a run. <span class="accent">See accuracy land.</span>` Subtitle: `The scheduler also triggers a run every 5 min so the page is never empty.`
- Form card: dropdown labelled "Suite" (populated from `GET /api/suites`), numeric field labelled "Passing threshold" (default 0.8), `Trigger Run` button (yellow).
- Live list: cards per run; left border coloured by status:
  - `PENDING` — muted grey
  - `RUNNING` — blue
  - `PASSED` — green
  - `FAILED` — red
- Header row: suite name, status pill, case counter (`n/totalCases evaluated`), pass rate chip.
- Click to expand: per-case timeline. Each case block shows:
  - **Case `caseId`** header with the input (first 80 chars) and the responded timestamp.
  - The raw output in a monospaced block.
  - The judge's verdict pill (`PASS` green, `FAIL` red) plus the confidence score badge.
  - The judge's notes (overallRationale + bullets) when present.
- Terminal block:
  - On `PASSED`: the final pass rate in a highlighted box plus "N of M cases passed."
  - On `FAILED`: the `failureReason` string plus the gate figures (passRate, threshold, failCount).
