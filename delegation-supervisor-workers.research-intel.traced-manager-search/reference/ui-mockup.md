...
Browser title: `<title>Akka Sample: Inspect Multi-Agent Run (Phoenix)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Inspect Multi-Agent Run (Phoenix)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose query into a search plan; SearchAgent fetches pages under a URL guardrail while Phoenix captures per-step spans; ManagerAgent synthesises an answer from results and a trace report.
...
A form with one `query` text input and a Submit button (`POST /api/runs`). Below it, a live list of runs streamed from `/api/runs/sse`. Each row shows the query, a status pill (DISPATCHED / SEARCHING / TRACED / PARTIAL / BLOCKED_TOOL), and the eval score when present. Click a row to expand:

- **Search results** — a list of page titles and excerpts.
- **Trace summary** — per-step token bars rendered as horizontal bar chart using inline SVG or CSS `width` percentage. Each bar shows `stepName`, total tokens for that step, and duration. The hot step is highlighted in yellow.
- **Synthesised answer** — the full answer text and source URLs.
- **Eval** — score (1–5 star icons or numeric) and rationale.

If a run is in `BLOCKED_TOOL`, show the blocked URL in a red callout instead of search results.
