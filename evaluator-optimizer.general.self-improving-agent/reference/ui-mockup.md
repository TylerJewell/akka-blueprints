# UI mockup — self-improving-agent
...
- Eyebrow: `Overview`.
- Headline: `Self-Improving <span class="accent">Agent</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a task in the App UI tab.
  2. Watch the configuration enter `IMPROVING`, then either `APPLIED` or `REVISION_REJECTED`.
  3. Expand the config to see every cycle's proposal, attestation verdict, and score delta.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: an executor processes tasks and records performance signals; an optimizer analyzes those signals and proposes a prompt revision; a regression attestation gate decides whether the revision is applied.
...
- ID badges coloured per mechanism (eval-event = blue, ci-gate = red, hotl = orange).
...
- Eyebrow: `App UI`. Headline: `Submit a task. <span class="accent">Watch the agent improve.</span>` Subtitle: `The simulator also drips a task every 60 s so the page is never empty.`
- Form card: text field labelled "Task instruction", text field labelled "Expected hint" (optional), `Submit` button (yellow).
- Live list: cards per agent configuration; left border coloured by status:
  - `ACTIVE` — muted grey
  - `IMPROVING` — blue
  - `APPLIED` — green
  - `REVISION_REJECTED` — red
- Header row: config ID (first 12 chars), status pill, revision counter (`n/maxRevisions`), best attestation score delta chip.
- Click to expand: per-cycle timeline. Each cycle block shows:
  - **Cycle n** header with the timestamp.
  - The proposed system prompt in a monospaced block (first 300 chars, with "show more" toggle).
  - The optimizer's rationale in a prose block.
  - The attestation verdict pill (`PASSED` green with score delta, `FAILED` red with score delta and detail).
- Terminal block:
  - On `APPLIED`: the applied system prompt in a highlighted box plus a "applied after N cycles" caption.
  - On `REVISION_REJECTED`: the best-scoring proposal's text plus the structured `rejectionReason`.
- Monitor badge: a persistent badge in the top-right of the App UI tab showing "Monitor connected" (green) or "Monitor disconnected" (muted) based on the `/api/configs/monitor/sse` connection state.
