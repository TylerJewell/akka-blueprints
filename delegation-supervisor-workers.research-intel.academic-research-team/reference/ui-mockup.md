...
Browser title: `<title>Akka Sample: Academic Research Team</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Academic Research Team", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose the query into a scouting directive and an interpretation question, parallel fan-out to PaperScout and DomainAnalyst, synthesise with a citation guardrail.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. The label column carries the control id and a coloured mechanism pill (`guardrail` red, `eval-event` blue). Click a row to expand name, rationale, and implementation. Two controls: G1, E1.

## Tab 5 — App UI

A form with one `query` text input and a Submit button (`POST /api/digests`). Below it, a live list of digests streamed from `/api/digests/sse`. Each row shows the query, a status pill (QUEUED / SCANNING / SYNTHESISED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand publications, trend report, the synthesised summary, and the eval rationale.
