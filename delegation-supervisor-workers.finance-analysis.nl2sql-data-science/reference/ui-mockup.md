...
Browser title: `<title>Akka Sample: Multi-Agent NL2SQL Data Science</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Multi-Agent NL2SQL Data Science", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: translate a financial question to SQL with a safety guardrail, parallel fan-out to DataRetriever + StatisticsModeler, synthesise into a data science report.
...
A form with one `question` text input and a Submit button (`POST /api/analysis`). Below it, a live list of jobs streamed from `/api/analysis/sse`. Each row shows the question (truncated at 80 characters), a status pill (TRANSLATING / RUNNING / COMPLETE / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand the SQL plan, the data table (column headers + rows), the model result metrics, the synthesised summary, and the eval rationale.
