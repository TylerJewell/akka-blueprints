...
Browser title: `<title>Akka Sample: Energy Efficiency Management Agent</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Energy Efficiency Management Agent", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: plan subsystem queries, parallel fan-out to HVAC + Lighting + Equipment specialists (each with a control-action guardrail), consolidate into one efficiency report.
...
A form with a `buildingId` text input, a `zone` text input, and a "Trigger Cycle" button (`POST /api/cycles`). Below it, a live list of cycles streamed from `/api/cycles/sse`. Each row shows the building + zone, a status pill (QUEUED / DISPATCHED / CONSOLIDATING / COMPLETE / PARTIAL / BLOCKED), and the eval score when present. Click a row to expand the HVAC report, lighting report, equipment report (with blocked actions highlighted), the consolidated summary, estimated savings in kWh, and the eval rationale.
