...
Browser title: `<title>Akka Sample: Startup Advisor (Multi-Agent)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Startup Advisor (Multi-Agent)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose a startup profile into four work items, parallel fan-out to MarketResearcher + GtmStrategist + ContentPlanner + RoadmapAdvisor, synthesise with an output guardrail.
...
A form with four inputs — `companyName`, `sector`, `stage` (dropdown: pre-idea / pre-seed / seed / Series A / Series B+), `problemStatement` (textarea) — and a Submit button (`POST /api/advisory`). Below it, a live list of advisory sessions streamed from `/api/advisory/sse`. Each row shows the company name, a status pill (PLANNING / IN_PROGRESS / SYNTHESISED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand the market landscape, GTM strategy, content plan, roadmap, synthesised executive summary, and eval rationale.
