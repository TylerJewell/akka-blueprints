...
Browser title: `<title>Akka Sample: Trip Planner Multi-Agent</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Trip Planner Multi-Agent", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose a trip request into research, itinerary, and booking tasks; run three specialists in parallel; gate the result through a budget guardrail and human approval before confirming.
...
A form with fields: destination (text), start date (date), end date (date), travellers (number), budget USD (number), and a Submit button (`POST /api/trips`). Below it, a live list of trips streamed from `/api/trips/sse`. Each row shows the destination, date range, a status pill (PLANNING / IN_PROGRESS / AWAITING_APPROVAL / CONFIRMED / REJECTED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand destination notes, itinerary days, booking proposals, the compiled plan summary, and the eval rationale. On `AWAITING_APPROVAL` rows, display Approve and Reject buttons that call the corresponding endpoints with the embedded approval token.
