...
5. **App UI** — submit an ops task description; live list of actions via SSE; per-action Approve/Reject buttons shown only when `status == PROPOSED` and `proposalSummary` is present. Executed actions show the outcome; rejected actions show the reason.
...
- **Task description input** — single text field + Submit button → `POST /api/action-request`.
- **Actions list** — one card per action: proposal summary, estimated impact, status badge, and (for `PROPOSED` actions) an approver-name field, a note field, Approve and Reject buttons. Reject reveals a reason field.
- **Status badge** — `PROPOSED` neutral, `APPROVED` yellow, `EXECUTED` green, `REJECTED` muted red.
- **Proposal detail** — `actionPayload` shown in a monospace block so reviewers can inspect the exact directive before deciding.
