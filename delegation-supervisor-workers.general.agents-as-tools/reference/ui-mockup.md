...
Browser title: `<title>Akka Sample: Agents as Tools</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Agents as Tools", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: supervisor receives task; selects writer_tool and/or data_tool; assembles and returns the result.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. Because this baseline has no wired controls, the tab displays an empty-state message:

> No controls are defined for this baseline. Add entries to `eval-matrix.yaml` to enable governance tracking.

The empty-state message is styled with the same `matrix-card` wrapper as populated rows, with muted text colour.

## Tab 5 — App UI

A form with one `description` text input and a Submit button (`POST /api/tasks`). Below it, a live list of task records streamed from `/api/tasks/sse`. Each row shows the task description (truncated to 80 chars), a status pill (QUEUED / PROCESSING / COMPLETED / FAILED / REJECTED), and the quality score when present. Click a row to expand: the drafted text (if `draft` is set), the extracted entities (if `data` is set), the assembled answer, and the quality rationale.
