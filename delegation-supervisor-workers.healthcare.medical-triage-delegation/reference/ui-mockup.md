...
Browser title: `<title>Akka Sample: Medical Agent Delegation</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Medical Agent Delegation", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: sanitize and classify incoming question; parallel fan-out to three specialists (symptoms, medications, care pathway); synthesise with a safety guardrail and flag for clinical review.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. The label column carries the control id and a colored mechanism pill (`sanitizer` purple, `guardrail` red, `hotl` orange). Click a row to expand name, rationale, and implementation. Three controls: S1, G1, H1.

## Tab 5 — App UI

A form with one `question` text input and a Submit button (`POST /api/triage`). Below it, a live list of cases streamed from `/api/triage/sse`. Each row shows the question (truncated to 80 chars), a status pill (TRIAGING / IN_REVIEW / RESPONDED / DEGRADED / BLOCKED), a "Pending review" badge when `hotlPending` is true, and the compliance score when present. Click a row to expand: symptoms assessment (urgency indicator + identified symptoms + differential summary), medication guidance (relevant medications + interactions + contraindications), care recommendation (recommended pathway + rationale + next steps), synthesised summary, failure reason if blocked, and compliance rationale.
