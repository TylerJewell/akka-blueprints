...
5. **App UI** — submit a task request; live list of tasks via SSE; per-task Approve/Reject buttons shown only when `status == PROCESSED` and `taskRecommendation` is present. Finalized tasks show the outcome; rejected tasks show the reason.
...
- **Request input** — single text field + Submit button → `POST /api/task-request`.
- **Tasks list** — one card per task: summary, status badge, recommendation text, and (for `PROCESSED` tasks) an approver-name field, a comment field, Approve and Reject buttons. Reject reveals a reason field.
- **Status badge** — `PROCESSED` neutral, `APPROVED` yellow, `FINALIZED` green, `REJECTED` muted red.
