...
Browser title: `<title>Akka Sample: Economic Research Agent</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Economic Research Agent", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: frame a market question, parallel fan-out to DataCollector + Economist, synthesise with a financial content guardrail.
...
A form with one `question` text input and a Submit button (`POST /api/analysis`). Below it, a live list of reports streamed from `/api/analysis/sse`. Each row shows the question, a status pill (FRAMING / IN_PROGRESS / PUBLISHED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand indicators, interpretation, the synthesised summary, and the eval rationale.
