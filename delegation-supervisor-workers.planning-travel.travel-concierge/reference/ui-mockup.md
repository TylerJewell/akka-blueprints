...
Browser title: `<title>Akka Sample: Travel Concierge</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Travel Concierge", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose the trip request, parallel research (destination + fare), assemble itinerary and hold for human approval before any booking.
- **Components** — the count: 3 autonomous-agent, 1 workflow, 2 event-sourced-entity, 1 view, 1 consumer, 1 timed-action, 2 http-endpoint.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. The label column carries the control id and a colored mechanism pill (`guardrail` red, `hitl` orange). Click a row to expand name, rationale, and implementation. Two controls: G1, H1.

## Tab 5 — App UI

A form with four inputs (destination text, departure date, return date, budget USD, traveller count) and a Submit button (`POST /api/trips`). Below the form, a live list of trips streamed from `/api/trips/sse`. Each row shows the destination, date range, a status pill (PLANNING / RESEARCHING / AWAITING_APPROVAL / CONFIRMED / DECLINED / DEGRADED / BLOCKED), and the budget vs fare when available. Click a row to expand the destination brief, fare estimate, and assembled itinerary. When status is `AWAITING_APPROVAL`, the expanded row shows a **Confirm** button (`POST /api/trips/{id}/confirm`) and a **Decline** button (`POST /api/trips/{id}/decline`).
