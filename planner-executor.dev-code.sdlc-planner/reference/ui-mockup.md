...
- Eyebrow: `Overview`.
- Headline: `SDLC Task <span class="accent">Planner</span>`. **No subtitle.**
- Card **Try it**: a single code block reading `/akka:build`. Below it, three numbered steps — submit an SDLC request in the App UI tab, watch the planner iterate through specialists, expand the row to see both ledgers and the final deliverable.
- Card **How it works**: one paragraph naming the components, the loop steps, and the four specialists.
...
- ID badges coloured per mechanism: `guardrail` red (`G1`), `halt` red (`HT1`), `hotl` muted (`HO1`), `sanitizer` green (`S1`).
- Rows expand vertically on click; one open at a time.

## Tab 5 — App UI

- Eyebrow: `App UI`. Headline: `Submit a request. <span class="accent">Watch the loop run.</span>` Subtitle: `The simulator drips a request every 90 s so the page is never empty.`
- Form card: text field labelled "SDLC request", `Submit` button (yellow).
...
- Live list: cards per plan; left border coloured by status (PLANNING = muted, EXECUTING = blue, COMPLETED = green, FAILED = red, HALTED = orange, STUCK = pale red).
  - Header row: prompt (first 80 chars), status pill, attempt count, elapsed time.
  - Click to expand:
    - **Task ledger**: requirements (yellow list), constraints (muted list), plan (numbered list), currentDispatch (specialist pill + subtask line).
    - **Progress ledger**: vertical timeline of `ProgressEntry` rows. Each row shows attempt number, specialist pill (`ANALYST` teal, `ARCHITECT` blue, `CODER` purple, `REVIEWER` green), subtask, verdict pill (`OK` green, `BLOCKED_BY_GUARDRAIL` yellow, `FAILED` red, `UNSAFE` dark red), and a collapsed `<pre>` of `scrubbedResult` (click to expand). Redacted spans render in italics with a tooltip showing the redaction tag (e.g., `[REDACTED:aws-access-key]`).
    - **Deliverable** (only when status is COMPLETED): summary paragraph + artefact bullets.
