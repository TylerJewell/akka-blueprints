# UI mockup — evaluator-optimizer-loop
...
- Eyebrow: `Overview`.
- Headline: `Evaluator-Optimizer <span class="accent">Loop</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a problem statement in the App UI tab.
  2. Watch the job enter `GENERATING`, then `EVALUATING`, then either `ACCEPTED` or `REJECTED_FINAL`.
  3. Expand the job to see every attempt's candidate, the guardrail verdict, the evaluator's verdict, and the evaluator's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a generator produces a candidate, an evaluator scores it, the workflow loops until convergence or the attempt ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a problem. <span class="accent">Watch a solution land.</span>` Subtitle: `The simulator also drips a problem every 60 s so the page is never empty.`
- Form card: text field labelled "Problem statement", numeric field labelled "Token ceiling" (default 500), `Submit` button (yellow).
- Live list: cards per job; left border coloured by status:
  - `GENERATING` — muted grey
  - `EVALUATING` — blue
  - `ACCEPTED` — green
  - `REJECTED_FINAL` — red
- Header row: description (first 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The candidate text in a monospaced block.
  - The guardrail verdict pill (`OK` green, `OVER_CEILING` red with the detail text).
  - The evaluator's verdict pill (`ACCEPT` green, `REVISE` orange) plus the score badge.
