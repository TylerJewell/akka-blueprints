...
Browser title: `<title>Akka Sample: Dev Team Task Board</title>`.
...
- Eyebrow: `Overview`.
- Headline: `Dev Team <span class="accent">Task Board</span>`. **No subtitle.**
- Card **Try it**: a `/akka:build` (Claude Code) block, then three numbered steps (submit a project, watch the team lead fill the board, watch developers claim and complete tasks). No env-var export block — `/akka:specify` handled the key during generation.
...
- ID badges carry a colored mechanism pill: guardrail = red, ci-gate = pale yellow, halt = red.
- Rows expand vertically on click to show rationale + implementation; one open at a time.

## Tab 5 — App UI

- Eyebrow: `App UI`. Headline: `Submit a project. <span class="accent">Watch the team build it.</span>` Subtitle: `The simulator also drips a project every 60 s so the board is never empty.`
- Form card: a "Project title" field, a "Description" textarea, a `Submit` button (yellow).
...
- The board: one column per `TaskStatus` (Open · Claimed · In progress · In review · Done · Blocked). Each task is a card showing its title, the project it belongs to, the claiming developer (when claimed), the artifact summary (when present), and the test result chip (green pass / red fail). A `BLOCKED` card shows the block reason. Cards move between columns live via SSE.
- Peer mailbox panel: a per-developer list of `PeerMessage` items; an unanswered message shows a reply box. Posting a reply unblocks the waiting task.
