...
Browser title: `<title>Akka Sample: Multi-Agent Supply Chain Optimizer</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Multi-Agent Supply Chain Optimizer", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose the demand signal, parallel fan-out to StockAnalyst + LogisticsPlanner, synthesise with an output guardrail.
...
A form with a `sku` text input, a `targetQuantity` number input, and a Submit button (`POST /api/supply`). Below it, a live list of orders streamed from `/api/supply/sse`. Each row shows the SKU, target quantity, a status pill (PENDING / ANALYZING / RECOMMENDED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand the stock assessment positions, route plan segments, the synthesised summary, and the eval rationale.
