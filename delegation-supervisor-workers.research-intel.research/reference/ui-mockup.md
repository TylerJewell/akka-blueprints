...
Browser title: `<title>Akka Sample: Multi-Agent Research Brief</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Multi-Agent Research Brief", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose, parallel fan-out to Researcher + Analyst, synthesise with an output guardrail.
...
A form with one `topic` text input and a Submit button (`POST /api/research`). Below it, a live list of briefs streamed from `/api/research/sse`. Each row shows the topic, a status pill (PLANNING / IN_PROGRESS / SYNTHESISED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand findings, analysis, the synthesised summary, and the eval rationale.
