# UI mockup — structured-output-reflection
...
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a generator produces JSON, a critic validates it, the workflow loops until the output passes both schema validation and semantic quality checks or the retry ceiling is reached.
...
- Form card: dropdown labelled "Schema" (options: product, event-record, contact), text area labelled "Prompt" (default placeholder "Describe the content to generate…"), `Submit` button (yellow).
