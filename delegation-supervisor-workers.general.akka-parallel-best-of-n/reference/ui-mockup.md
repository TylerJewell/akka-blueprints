...
Browser title: `<title>Akka Sample: Parallel Best-of-N</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Parallel Best-of-N", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: supervisor plans N variant instructions, parallel workers produce one translation each, supervisor selects best with an output guardrail.
...
A form with a `sourceText` textarea, a `targetLanguage` text input, and a Submit button (`POST /api/translation`). Below it, a live list of jobs streamed from `/api/translation/sse`. Each row shows the source text (truncated to 60 chars), the target language, a status pill (PENDING / IN_PROGRESS / SELECTED / PARTIAL / BLOCKED), and the eval score when present. Click a row to expand all returned variants with their register labels, the selected result with rationale, and the eval rationale when available.
