...
- Eyebrow: `Overview`.
- Headline: `On-Brand <span class="accent">GenMedia</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a campaign brief in the App UI tab.
  2. Watch the asset enter `GENERATING`, then `REVIEWING`, then either `APPROVED` or `REJECTED_FINAL`.
  3. Expand the asset to see every attempt's generated copy, the guardrail verdict, the reviewer's verdict, and the reviewer's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a brand agent generates copy, a reviewer scores it, the workflow loops until approval or the retry ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a brief. <span class="accent">Watch an asset land.</span>` Subtitle: `The simulator also drips a brief every 60 s so the page is never empty.`
- Form card: text field labelled "Product", text field labelled "Channel" (default "LinkedIn"), text field labelled "Tone" (default "professional"), numeric field labelled "Token ceiling" (default 300), `Submit` button (yellow).
- Live list: cards per asset; left border coloured by status:
  - `GENERATING` — muted grey
  - `REVIEWING` — blue
  - `APPROVED` — green
  - `REJECTED_FINAL` — red
- Header row: product + channel (truncated to 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The headline and body copy in a monospaced block, plus the social caption in a lighter block.
...
- Terminal block:
  - On `APPROVED`: the approved headline, body copy, and social caption in a highlighted box plus a "best of N attempts" caption.
  - On `REJECTED_FINAL`: the highest-scoring attempt's asset plus the structured `rejectionReason`.
