...
Browser title: `<title>Akka Sample: Orchestrator-Workers Workflow</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Orchestrator-Workers Workflow", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose task into per-file instructions, parallel fan-out to FileEditor workers (each tool call gated by a before-tool-call guardrail), synthesise changeset with EditReviewer verdicts.
...
A form with a `description` text input, a `targetFiles` text area (one path per line), and a Submit button (`POST /api/tasks`). Below it, a live list of tasks streamed from `/api/tasks/sse`. Each row shows the description (truncated to 80 chars), a status pill (PLANNING / IN_PROGRESS / COMPLETED / DEGRADED / BLOCKED), and the quality score when present. Click a row to expand the decomposition plan, the per-file edits with their diff summaries, the review verdicts, the changeset summary, and the quality rationale.
