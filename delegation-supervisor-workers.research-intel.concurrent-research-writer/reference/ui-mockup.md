...
Browser title: `<title>Akka Sample: Concurrent Research Writer</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 -- Overview

Eyebrow "Overview" + headline "Concurrent Research Writer", **no subtitle**. Four cards:

- **Try it** -- the single command `/akka:build`. No env-var export block.
- **How it works** -- three lines: decompose, parallel fan-out to SourceResearcher + DraftWriter, assemble with an output guardrail.
...
A form with one `topic` text input and a Submit button (`POST /api/reports`). Below it, a live list of reports streamed from `/api/reports/sse`. Each row shows the topic, a status pill (PLANNING / IN_PROGRESS / ASSEMBLED / DEGRADED / BLOCKED), and the quality score when present. Click a row to expand sources, draft section, the assembled report body, and the quality rationale.
