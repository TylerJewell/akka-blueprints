...
- Eyebrow: `Overview`.
- Headline: `Scored CV <span class="accent">Reviewer</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a job posting in the App UI tab.
  2. Watch the application enter `TAILORING`, then `REVIEWING`, then either `APPROVED` or `REJECTED_FINAL`.
  3. Expand the application to see every attempt's CV draft, the guardrail verdict, the reviewer's verdict, and the reviewer's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a CV tailor drafts, a reviewer scores, the workflow loops until convergence or the retry ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a job posting. <span class="accent">Watch a CV land.</span>` Subtitle: `The simulator also drips a posting every 60 s so the page is never empty.`
- Form card: text field labelled "Job title" (required), textarea labelled "Job description", textarea labelled "Candidate profile (optional)", `Submit` button (yellow).
- Live list: cards per application; left border coloured by status:
  - `TAILORING` — muted grey
  - `REVIEWING` — blue
  - `APPROVED` — green
  - `REJECTED_FINAL` — red
- Header row: job title (first 60 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The CV draft text in a monospaced block.
  - The guardrail verdict pill (`OK` green, `MISSING_SECTIONS` red with the detail text).
...
- Terminal block:
  - On `APPROVED`: the approved CV text in a highlighted box plus a "best of N attempts" caption.
  - On `REJECTED_FINAL`: the highest-scoring draft's text plus the structured `rejectionReason`.
