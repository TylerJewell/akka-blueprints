# UI mockup — sandboxed-analyst-agent
...
The DOM contains **exactly five** `<section class="tab-panel">` elements — Overview, Architecture, Risk Survey, Eval Matrix, App UI — no more. Any panel removed in an earlier iteration must be deleted from the HTML; `display:none` is not sufficient. See the exemplar lessons on the failure mode this prevents.

## Mermaid CSS overrides (Lesson 24)

The `<style>` block in `static-resources/index.html` includes CSS overrides AND `themeVariables` — state-diagram label colour, edge-label `foreignObject overflow:visible`, `transitionLabelColor #cccccc`. Without these the state-machine diagram on the Architecture tab renders state names invisible and edge labels clip.

## Tab 1 — Overview

- Eyebrow: `Overview`.
- Headline: `Sales Call Data <span class="accent">Analyzer</span>`. **No subtitle.**
- Card **Try it**: a single code block reading `/akka:build`. Below it, three numbered steps — upload a dataset in the App UI tab, watch the analyst plan and execute steps, expand the row to see both ledgers and the final report.
- Card **How it works**: one paragraph naming the components, the loop steps, and the two agents.
...
- ID badges coloured per mechanism: `guardrail` red (`G1`), `halt` red (`HT1`), `sanitizer` green (`S1`).
- Rows expand vertically on click; one open at a time.

## Tab 5 — App UI

- Eyebrow: `App UI`. Headline: `Upload a dataset. <span class="accent">Watch the analysis run.</span>` Subtitle: `The simulator drips a dataset every 90 s so the page is never empty.`
- Form card: text field labelled "Dataset name", optional textarea for CSV content, `Upload` button (yellow).
...
- Live list: cards per job; left border coloured by status (PLANNING = muted, EXECUTING = blue, COMPLETED = green, FAILED = red, HALTED = orange, STUCK = pale red).
  - Header row: dataset name (first 80 chars), status pill, step count, elapsed time.
  - Click to expand:
    - **Analysis ledger**: dataset facts (yellow list), hypotheses (muted list), plan (numbered list), currentStep (script kind pill + script preview collapsible).
    - **Progress ledger**: vertical timeline of `ProgressEntry` rows. Each row shows attempt number, script kind pill (`LOAD_INSPECT` blue, `AGGREGATE` green, `SENTIMENT` purple, `SEGMENT` cyan, `VISUALISE` orange, `SUMMARISE` muted), verdict pill (`OK` green, `BLOCKED_BY_GUARDRAIL` yellow, `FAILED` red, `UNSAFE` dark red), and a collapsed `<pre>` of `scrubbedOutput` (click to expand). Redacted spans render in italics with a tooltip showing the redaction tag (e.g., `[REDACTED:phone]`).
    - **Report** (only when status is `COMPLETED`): summary paragraph + findings table (metricName / value / interpretation) + charts list.
