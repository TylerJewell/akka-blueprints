...
Browser title: `<title>Akka Sample: Haiku Sub-Agents under Opus (multimodal)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Haiku Sub-Agents under Opus (multimodal)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: (1) Opus decomposes the image-batch question into per-image instructions; (2) Haiku sub-agents analyse each image in parallel; (3) Opus synthesises all reports with an output guardrail.
- **Components** — the count: 2 autonomous-agent (1 Opus + 1 Haiku), 1 workflow, 1 sanitizer component, 2 event-sourced-entity, 1 view, 1 consumer, 2 timed-action, 2 http-endpoint.
...
A form with a `question` text input and a multi-file image input (or URL list as a `<textarea>`), and a Submit button (`POST /api/analysis`). Below it, a live list of analysis jobs streamed from `/api/analysis/sse`. Each row shows the question (truncated to 60 chars), an image count badge, a status pill (QUEUED / ANALYSING / SYNTHESISED / PARTIAL / BLOCKED), and the eval score when present. Click a row to expand: the per-image report cards (each showing the `imageRef`, description, tags as chips, and detected labels with confidence bars), the synthesised answer, and the eval rationale.
