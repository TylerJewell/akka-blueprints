...
4. **Eval Matrix** — fetches `/api/metadata/eval-matrix`, renders one `matrix-row` per control. The label column carries the control id plus a colored mechanism pill (`sanitizer` blue, `hitl` yellow, `guardrail` red). The value column shows name, rationale, and implementation.
5. **App UI** — submit an entity id and source documents; live list of cases via SSE; per-case Approve/Reject buttons shown only when `status == SCREENED` and `findings` is present. Closed cases show the disposition; rejected cases show the reason.
...
- **Entity id input** — single text field for the entity identifier.
- **Documents input** — textarea accepting one source document excerpt per line.
- **Submit button** — `POST /api/screening-request` with entityId and documents array.
- **Cases list** — one card per case: entity id, status badge, findings summary, recommendation badge, and (for `SCREENED` cases) an officer-name field, a note field, Approve and Reject buttons. Reject reveals a reason field.
- **Status badge** — `SCREENED` neutral, `OFFICER_APPROVED` yellow, `CLOSED` green, `REJECTED` muted red.
- **Recommendation badge** — `PASS` green, `REFER` yellow, `BLOCK` red.
