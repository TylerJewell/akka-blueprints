...
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a generator produces an answer, a judge scores it, the workflow loops until the judge accepts or the retry ceiling is reached.
...
- Form card: textarea labelled "Question", text field labelled "Domain tag" (default `general`), numeric field labelled "Score threshold" (default 4, range 1–5), `Submit` button (yellow).
