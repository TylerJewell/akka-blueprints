# UI mockup — multi-turn-simulator
...
- Eyebrow: `Overview`.
- Headline: `Multi-Turn Actor <span class="accent">Simulator</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a scenario (persona + goal) in the App UI tab.
  2. Watch the session enter `RUNNING` as each turn is processed.
  3. Expand the session to see every turn's utterance, target response, guardrail verdict, and evaluator score.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: an actor agent generates user turns, a proxy forwards them to the target agent, an evaluator scores each response, and the workflow loops until goal completion or the turn ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a scenario. <span class="accent">Watch a session run.</span>` Subtitle: `The simulator also drips a scenario every 90 s so the page is never empty.`
- Form card: text field labelled "Persona", text field labelled "Goal", numeric field labelled "Max turns" (default 10), `Submit` button (yellow).
- Live list: cards per session; left border coloured by status:
  - `RUNNING` — blue
  - `COMPLETED` — green
  - `FLAGGED` — orange
  - `HALTED` — muted grey
- Header row: goal (first 60 chars), status pill, turn counter (`n/maxTurns`), session score chip.
- Click to expand: per-turn timeline. Each turn block shows:
  - **Turn n** header with the timestamp.
  - The actor's utterance in a left-aligned speech bubble.
  - The target agent's response in a right-aligned speech bubble.
  - The guardrail verdict pill (`OK` green, `POLICY_VIOLATION` red with detail text).
  - The evaluator's verdict pill (`PASS` green, `REVISE` orange, `POLICY_VIOLATION` red) plus the score badge.
  - The evaluator's rationale and dimension scores when present.
- Terminal block:
  - On `COMPLETED`: the session verdict box with `sessionScore`, `CLEAN` badge, and summary rationale.
  - On `FLAGGED`: the session verdict box with `FLAGGED` badge, drift indicator, and summary rationale.
  - On `HALTED`: the halt reason and the last scored turn's text.
