...
Browser title: `<title>Akka Sample: Real Estate Investment Multi-Agent</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Real Estate Investment Multi-Agent", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: sector sanitizer screens the submission, parallel fan-out to MarketSpecialist + FinanceSpecialist, synthesise with an output guardrail.
...
A form with three inputs: address (text), property type (select: multifamily / single-family / commercial / industrial / land), asking price (number), and a Submit button (`POST /api/deals`). Below it, a live list of deals streamed from `/api/deals/sse`. Each row shows the address, property type, a status pill (SCOPING / EVALUATING / RECOMMENDED / DEGRADED / BLOCKED / REJECTED), and the eval score when present. Click a row to expand: market report comparables table, financial model metrics (cap rate, NOI, cash-on-cash, DSCR), the synthesised recommendation summary, and the eval rationale.
