...
Browser title: `<title>Akka Sample: Marketing Agency Team</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Marketing Agency Team", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose a campaign brief, parallel fan-out to WebsiteLauncher + StrategyAdvisor, synthesise with a brand-safety guardrail.
...
A form with a `campaignName` text input, an `objective` text input, and a Submit button (`POST /api/campaigns`). Below it, a live list of plans streamed from `/api/campaigns/sse`. Each row shows the campaign name, a status pill (PLANNING / IN_PROGRESS / SYNTHESISED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand the launch checklist items, messaging pillars, the synthesised executive summary, and the eval rationale.
