# UI mockup — writer-reviewer-doc-gen
...
- Eyebrow: `Overview`.
- Headline: `SK Document <span class="accent">Generator</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a topic in the App UI tab.
  2. Watch the document enter `DRAFTING`, then `REVIEWING`, then either `APPROVED` or `REJECTED_FINAL`.
  3. Expand the document to see every attempt's draft, the guardrail verdict, the reviewer's verdict, and the reviewer's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a writer produces a structured document, a reviewer evaluates it, the workflow loops until convergence or the retry ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a topic. <span class="accent">Watch a document land.</span>` Subtitle: `The simulator also drips a topic every 60 s so the page is never empty.`
- Form card: text field labelled "Topic", numeric field labelled "Word ceiling" (default 500), `Submit` button (yellow).
- Live list: cards per document; left border coloured by status:
  - `DRAFTING` — muted grey
  - `REVIEWING` — blue
  - `APPROVED` — green
  - `REJECTED_FINAL` — red
...
- Click to expand: per-attempt timeline. Each attempt block shows:
  ...
  - The draft text in a monospaced block.
...
- Terminal block:
  - On `APPROVED`: the approved text in a highlighted box plus a "best of N attempts" caption.
  - On `REJECTED_FINAL`: the best-scoring draft's text plus the structured `rejectionReason`.
