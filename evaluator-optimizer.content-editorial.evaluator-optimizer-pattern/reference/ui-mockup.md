# UI mockup — evaluator-optimizer-workflow
...
- Eyebrow: `Overview`.
- Headline: `Evaluator-Optimizer <span class="accent">Workflow</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit an article brief in the App UI tab.
  2. Watch the headline enter `DRAFTING`, then `REVIEWING`, then either `APPROVED` or `REJECTED_FINAL`.
  3. Expand the headline to see every attempt's draft, the guardrail verdict, the reviewer's verdict, and the reviewer's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a writer drafts, a reviewer scores, the workflow loops until convergence or the retry ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a brief. <span class="accent">Watch a headline land.</span>` Subtitle: `The simulator also drips a brief every 60 s so the page is never empty.`
- Form card: textarea labelled "Article summary", numeric field labelled "Word ceiling" (default 12), `Submit` button (yellow).
- Live list: cards per headline; left border coloured by status:
  - `DRAFTING` — muted grey
  - `REVIEWING` — blue
  - `APPROVED` — green
  - `REJECTED_FINAL` — red
- Header row: summary (first 80 chars), status pill, attempt counter (`n/maxAttempts`), best-score chip.
- Click to expand: per-attempt timeline. Each attempt block shows:
  - **Attempt n** header with the timestamp.
  - The headline text in a monospaced block.
  - The guardrail verdict pill (`OK` green, `OVER_CEILING` red with the detail text).
...
- Terminal block:
  - On `APPROVED`: the approved headline text in a highlighted box plus a "best of N attempts" caption.
  - On `REJECTED_FINAL`: the best-scoring attempt's headline text plus the structured `rejectionReason`.
