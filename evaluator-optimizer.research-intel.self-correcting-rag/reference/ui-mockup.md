...
- Eyebrow: `Overview`.
- Headline: `Adaptive/Corrective <span class="accent">RAG</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a research question in the App UI tab.
  2. Watch the query move through `RETRIEVING` → `GRADING` → `GENERATING` → `HALLUCINATION_CHECK` → `ANSWERED` (or a failure state if no relevant documents are found or the answer cannot be grounded).
  3. Expand the query to see each retrieval pass, the relevance grades per document, the generated answer, and the hallucination verdict.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a retriever fetches documents, a grader filters them, a rewriter adapts the query on miss, a generator answers, and a hallucination grader gates the answer before surfacing it.
...
- Eyebrow: `App UI`. Headline: `Ask a research question. <span class="accent">Get a grounded answer.</span>` Subtitle: `The simulator also drips a query every 60 s so the page is never empty.`
...
- Live list: cards per query; left border coloured by status:
  ...
  - `REWRITING` — orange
  - `GENERATING` — purple
  - `HALLUCINATION_CHECK` — amber
  - `ANSWERED` — green
  - `FAILED_NO_RELEVANT_DOCS` — red
  - `FAILED_HALLUCINATION` — red
- Header row: query text (first 80 chars), status pill, pass counter (`n passes`), retrieval count chip.
- Click to expand: per-pass timeline. Each pass block shows:
  - **Pass n** header with the timestamp.
  - The documents retrieved (title + docId), each with its relevance verdict pill (`RELEVANT` green, `IRRELEVANT` red) and score.
  - If a rewrite occurred: the rewritten query text and the rationale.
- Generation block (below retrieval passes):
  - The generated answer text.
  - Cited document IDs as pills.
  - The hallucination verdict pill (`GROUNDED` green, `HALLUCINATED` red) with the rationale.
  - On `HALLUCINATED`: the unsupported claims in a warning box.
- Terminal block:
  - On `ANSWERED`: the grounded answer in a highlighted box.
  - On `FAILED_NO_RELEVANT_DOCS`: the failure reason and the last rewritten query tried.
  - On `FAILED_HALLUCINATION`: the failure reason and the best-attempt answer with the unsupported claims still flagged.
