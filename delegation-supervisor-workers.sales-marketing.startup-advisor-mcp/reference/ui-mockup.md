...
Browser title: `<title>Akka Sample: Startup Advisor (MCP)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Startup Advisor (MCP)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose the startup brief, parallel fan-out to MarketResearcher (MCP market-data tools) + GrowthAnalyst (MCP channel-scoring tools), synthesise GTM guidance with a tool allow-list guardrail protecting every tool call.
...
A form with one `description` textarea and a Submit button (`POST /api/advisory`). Below it, a live list of sessions streamed from `/api/advisory/sse`. Each row shows the brief description (truncated to 80 chars), a status pill (SCOPING / IN_PROGRESS / ADVISED / DEGRADED), and the eval score when present. Click a row to expand the market snapshot (TAM, competitors, buyer segments), channel plan (recommended channels with effort levels), the GTM guidance summary, and the eval rationale.
