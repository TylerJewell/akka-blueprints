...
Browser title: `<title>Akka Sample: Multi-Agent Collaboration (Supervisor)</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Multi-Agent Collaboration (Supervisor)", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: supervisor classifies and routes the task, parallel fan-out to ResearchWorker and ChartWorker, synthesise with a before-tool-call guardrail.
...
A form with one `description` text input and a Submit button (`POST /api/tasks`). Below it, a live list of tasks streamed from `/api/tasks/sse`. Each row shows the description (truncated to 80 chars), a status pill (ROUTING / IN_PROGRESS / COMPLETED / DEGRADED / BLOCKED), and the eval score when present. Click a row to expand: research findings list, chart data (rendered as an inline bar or line chart using canvas or SVG), the synthesised summary, and the eval rationale.
