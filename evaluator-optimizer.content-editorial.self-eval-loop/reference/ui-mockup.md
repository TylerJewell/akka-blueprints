...
- Eyebrow: `Overview`.
- Headline: `Self-Eval <span class="accent">Loop Posts</span>`. **No subtitle.**
- Card **Try it** — one block: `/akka:build`. Then three numbered steps:
  1. Submit a brief in the App UI tab.
  2. Watch the post enter `DRAFTING`, then `EVALUATING`, then either `ACCEPTED` or `REJECTED_FINAL`.
  3. Expand the post to see every attempt's draft, the guardrail verdict, the critic's verdict, and the critic's notes.
- Card **How it works** — one paragraph naming the evaluator-optimizer pattern: a poet drafts, a critic scores, the workflow loops until convergence or the retry ceiling.
...
- Eyebrow: `App UI`. Headline: `Submit a brief. <span class="accent">Watch a passage land.</span>` Subtitle: `The simulator also drips a brief every 60 s so the page is never empty.`
- Form card: text field labelled "Brief", numeric field labelled "Character ceiling" (default 280), `Submit` button (yellow).
- Live list: cards per post; left border coloured by status:
  - `DRAFTING` — muted grey
  - `EVALUATING` — blue
  - `ACCEPTED` — green
  - `REJECTED_FINAL` — red
...
- Click to expand: per-attempt timeline. Each attempt block shows:
  ...
  - The critic's verdict pill (`ACCEPT` green, `REVISE` orange) plus the score badge.
  - The critic's notes (overallRationale + bullets) when present.
- Terminal block:
