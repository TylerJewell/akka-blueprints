...
Browser title: `<title>Akka Sample: Hiring Supervisor Orchestration</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Hiring Supervisor Orchestration", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: sanitize application and plan evaluation, parallel fan-out to ScreeningAgent + SchedulingAgent, consolidate with a before-invocation policy check.
...
Renders `/api/metadata/eval-matrix` in `matrix-card` / `matrix-row` style. The label column carries the control id and a colored mechanism pill (`guardrail` red, `sanitizer` orange). Click a row to expand name, rationale, and implementation. Two controls: G1, S1.

## Tab 5 — App UI

A form with four inputs — `candidateId`, `roleId`, `resumeText` (textarea), `availabilityWindow` — and a Submit button (`POST /api/hiring`). Below it, a live list of applications streamed from `/api/hiring/sse`. Each row shows the candidateId, roleId, a status pill (RECEIVED / UNDER_REVIEW / RECOMMENDED / REJECTED / DEGRADED / POLICY_HOLD), and the eval score when present. Click a row to expand the screening report (qualificationScore + strengthHighlights + gapNotes), scheduling proposal (proposed slots), the hiring recommendation rationale, and the eval rationale.
