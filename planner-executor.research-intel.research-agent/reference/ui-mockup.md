...
The DOM contains **exactly five** `<section class="tab-panel">` elements. Any panel removed in an earlier iteration must be deleted from the HTML; `display:none` is not sufficient. See Lesson 26.
...
- Eyebrow: `Overview`.
- Headline: `Research <span class="accent">Agent</span>`. **No subtitle.**
- Card **Try it**: a single code block reading `/akka:build`. Below it, three numbered steps — submit a research query in the App UI tab, watch the planner iterate, expand the row to see the search ledger, findings with citation verdicts, and the final report.
- Card **How it works**: one paragraph naming the components, the loop steps, and the two searchers.
...
- Each `.qb` rendered with the question text and chips/widgets in their selected state per `risk-survey.yaml`.
...
- Eyebrow: `App UI`. Headline: `Submit a query. <span class="accent">Watch the research run.</span>` Subtitle: `The simulator drips a query every 90 s so the page is never empty.`
- Form card: text field labelled "Research query", `Submit` button (yellow).
...
- Live list: cards per job; left border coloured by status (PLANNING = muted, SEARCHING = blue, COMPLETED = green, FAILED = red, HALTED = orange, STUCK = pale red).
  - Header row: query (first 80 chars), status pill, finding count, elapsed time.
  - Click to expand:
    - **Search ledger**: context paragraph, knownFacts (yellow list), missingFacts (muted list), plan (numbered list), currentDispatch (searcher pill + query line).
    - **Findings ledger**: vertical timeline of `FindingEntry` rows. Each row shows attempt number, searcher pill (`WEB` blue, `DOCUMENT` green), query text, citation verdict chip (`CITED` green with confidence %, `UNCITED` yellow), and a collapsed `<pre>` of content (click to expand).
    - **Report** (only when status is `COMPLETED`): title, summary paragraph, findings list, and a citations block. Each citation shows the source label, searcher, and source signal type.
