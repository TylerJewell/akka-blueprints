...
5. **App UI** — submit a topic; live list of posts via SSE; per-post Approve/Reject buttons shown only when `status == DRAFTED` and `draftContent` is present. Published posts show the URL; rejected posts show the reason.
...
- **Topic input** — single text field + Submit button → `POST /api/publish-request`.
- **Posts list** — one card per post: title, status badge, content preview, and (for `DRAFTED` posts) an approver-name field, a comment field, Approve and Reject buttons. Reject reveals a reason field.
- **Status badge** — `DRAFTED` neutral, `APPROVED` yellow, `PUBLISHED` green, `REJECTED` muted red.
