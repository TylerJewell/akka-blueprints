...
Browser title: `<title>Akka Sample: Retail AI Location Strategy</title>`.

Five left-nav tabs. Each `.nav-tab` carries `data-tab="N"`; each `.tab-panel` carries `data-panel="N"`.

## Tab 1 — Overview

Eyebrow "Overview" + headline "Retail AI Location Strategy", **no subtitle**. Four cards:

- **Try it** — the single command `/akka:build`. No env-var export block.
- **How it works** — three lines: decompose a candidate site address into scoring tasks, parallel scoring by Market Analyst + Demographics Analyst, synthesise with an output guardrail into a ranked recommendation.
...
A form with three inputs — `address`, `city`, `region` — and a Submit button (`POST /api/locations`). Below it, a live list of site evaluations streamed from `/api/locations/sse`. Each row shows the address + city, a status pill (SCORING / IN_PROGRESS / RECOMMENDED / NOT_RECOMMENDED / DEGRADED / BLOCKED), the composite score when present, and the eval score badge when available. Click a row to expand the market assessment key insights, demographic assessment key insights, the synthesised recommendation summary, and the eval rationale.
